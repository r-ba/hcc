...
impl Ast {
    ...
    pub fn from_pairs(mut pairs: Pairs<Rule>) -> Result<Ast, Vec<AstError>> {
        ...
        let mut methods = vec![];
        let _ = context.idstore.get_id("main");
        ...
        .unwrap();
        let mut free = Function::from_pair(
            CParser::parse(Rule::function_header, "i0 freebird(I64* a);")
                .unwrap()
                .next()
                .unwrap(),
            &mut context,
        ...
        context
            .functions
            .entry(context.idstore.get_id("Free"))
            .or_insert_with(|| vec![])
            .insert(0, Rc::new(free));
        context
            .functions
